...
class Pc(config: AdeptConfig, br: BranchOpConstants) extends Module{
  val io = IO(new Bundle {
    ...
    // In from decoder
    val in_opcode = Input (UInt (10.W)) // opecode(7 bits) + function(3 bits) from word
    // Jump Adress in
  ...
  val branch_exe = (io.in_opcode (6, 4) === "b110".U) && (io.in_opcode(1,0) === "b11".U) // branch opecode verification
  ...
  val offset_sel = branch_exe &((io.in_opcode(3) & io.in_opcode(2)) | ((~(io.in_opcode (3) | io.in_opcode (2))) & Cond_Br_Ver))

  // Auxiliar variable that contains either offset or 4
  val add_to_pc_val = Mux (offset_sel, io.br_offset, 4.asSInt (config.XLen.W))

  // next pc calculation
  val next_pc = io.pc_out.asSInt + add_to_pc_val
  val jalrORpc_select = Mux ((branch_exe & (~io.in_opcode (3) & io.in_opcode (2))),io.br_step , next_pc)
  val Prog_Count = RegInit (0.S(config.XLen.W))
  Prog_Count := jalrORpc_select
  io.pc_out := Prog_Count.asUInt
}
